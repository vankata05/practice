...
void addEdge(graph_t* graph){
    graph->numOfEdges++;
    graph->graph = (bool**) realloc(graph->graph, graph->numOfEdges*sizeof(bool*));
    for(int i = 0; i < graph->numOfEdges; i++){
        graph->graph[i] = (bool*) realloc(graph->graph, graph->numOfEdges*sizeof(bool));
...
void dijkstra(graph_t graph, unsigned int to){
    ...
    for(int a = 0; a < graph.numOfEdges; a++){
        for(int b = 0; b < graph.numOfEdges; b++){ 
            if(graph.graph[a][b] == 1){
                if(edges[b][0] > edges[a][0] || edges[b][0] == -1){
                    if(edges[a][0] != -1){
                        if(edges[b][0] == -1)
                            edges[b][0] = 0;
